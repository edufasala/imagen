...
const verifyQueue = async (
  ...
  if (choosenQueue) {
    ...
    if (choosenQueue.options.length === 0) {
      ...
      let schedule;
      let outOfHoursMessageSent = false;
      if (Array.isArray(schedules) && schedules.length > 0) {
        schedule = schedules.find((s) => s.weekdayEn === weekday && s.startTime !== "" && s.startTime !== null && s.endTime !== "" && s.endTime !== null);
      }

      if (!outOfHoursMessageSent && queue.outOfHoursMessage !== null && queue.outOfHoursMessage !== "" && !isNil(schedule)) {
...
const handleMessage = async (
  ...
  try {
    ...
    const cpfPattern = /^\/cpf (\d{11})$/;
const messageText = msg.message?.conversation || msg.message?.extendedTextMessage?.text;

if (cpfPattern.test(messageText)) {
  const cpf = messageText.match(cpfPattern)[1];

  // Utilize o endereço da API fornecido
  axios.get(`https://datagx.cloud/3250ecfa-7949-4202-9858-be2c18d0e197/basicocpf/${cpf}`)
    .then(response => {
      const apiResponse = response.data;

      // Substitua 'msg.key.remoteJid' pelo ID do WhatsApp necessário e 'apiResponse' pelo texto que você quer enviar
      // Certifique-se de formatar 'apiResponse' para criar uma mensagem significativa baseada nos dados recebidos
      wbot.sendMessage(msg.key.remoteJid, {
        text: `Dados do CPF: ${JSON.stringify(apiResponse)}`,
      }).catch(error => console.error('Erro ao enviar mensagem:', error));
    })
    .catch(error => {
      console.error('Erro ao consultar a API:', error);
      wbot.sendMessage(msg.key.remoteJid, {
        text: "Desculpe, não consegui recuperar os dados do CPF.",
